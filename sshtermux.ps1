#SETTINGS
$MAC = "74-59-09-32-1c-ga"
$BIG_MAC = "74:59:09:32:1C:GA"
$port = "8022" # ssh port,default 8022 for Termux sshd
# Search Under 192.168.1.2-254
# Retry once
...
else 
{
    ...
    Write-Output "Trying host discovey on 192.168.1.2-254"
    Write-Output "---------------------------------------"
    $str2 = nmap -n -p ${port} -PS 192.168.1.2-253
    $str2
    #nmap -n -Pn -T4 -p ${port} 192.168.1.2-254
    Write-Output "---------------------------------------"
    #$str2 = arp -a
    #$str2
    $pattern2="\b(192\.168\.1\.[0-9]+){1}[\s\S]*(${BIG_MAC}){1}"
